...
export default function QuizLibrary({ onQuizSelected }) {
  ...
  // Get all unique subjects dynamically
  const subjectsList = useMemo(() => {
    const list = new Set();
    quizzes.forEach((q) => {
      list.add(q.subject || 'Chủ nghĩa xã hội khoa học');
    });
    return ['Tất cả', ...Array.from(list)];
  }, [quizzes]);

  // Reset active tab if it's no longer in the list (e.g. after deletion)
  useEffect(() => {
    if (!subjectsList.includes(activeTab)) {
      setActiveTab('Tất cả');
    }
  }, [subjectsList, activeTab]);

  // Filter quizzes by active subject tab
  const filteredQuizzes = useMemo(() => {
    if (activeTab === 'Tất cả') return quizzes;
    return quizzes.filter((q) => (q.subject || 'Chủ nghĩa xã hội khoa học') === activeTab);
  }, [quizzes, activeTab]);

  const getSubjectCount = (sub) => {
    if (sub === 'Tất cả') return quizzes.length;
    return quizzes.filter((q) => (q.subject || 'Chủ nghĩa xã hội khoa học') === sub).length;
  };
  ...
  return (
    <div className="card glass-panel p-4 mb-4">
      ...
      {quizzes.length === 0 ? (
        ...
        <div>
          {/* Dynamic Subject Category Tabs */}
          <div className="d-flex align-items-center gap-2 mb-4 overflow-auto pb-2 scrollbar-none" style={{ whiteSpace: 'nowrap', WebkitOverflowScrolling: 'touch' }}>
            {subjectsList.map((sub) => {
              ...
              return (
                <button
                  ...
                  className={`btn btn-sm px-3 py-1.5 rounded-pill transition-all fw-semibold d-flex align-items-center gap-2 ${
                    isActive
                      ? 'bg-gradient-header text-white border-0 shadow-sm'
                      : 'btn-outline-secondary border-slate-800 text-slate-400'
                  }`}
                  style={{ fontSize: '12px' }}
                >
                  <span>{sub}</span>
          ...
          {filteredQuizzes.length === 0 ? (
            <div className="text-center py-5 text-slate-500 text-sm italic">
              Không có đề trắc nghiệm nào thuộc môn học này.
            </div>
          ) : (
            <div>
              {/* Desktop View */}
